...
def decode_text(decode_img_name):
    image = cv2.imread(decode_img_name)
    text = showData(image)
    return text


def steganography_encode(data_encode, encode_img_name):
    encode_text(data_encode, encode_img_name)


def steganography_decode(decode_img_name):
    return decode_text(decode_img_name)
